...
class JavaEcosystemManager:
    ...
    def _init_maven_manager(self):
        ...
        try:
            self.maven_manager = MavenManager(project_dir=self.project_dir, logger=self.logger)
    ...
    def _init_gradle_manager(self):
        ...
        try:
            self.gradle_manager = GradleManager(project_dir=self.project_dir, logger=self.logger)
    ...
    def analyze_project(self) -> JavaEcosystemAnalysis:
        ...
        """
        analysis = JavaEcosystemAnalysis(build_system='Unknown', is_java_project=self.is_java_project())
        ...
        try:
            detector = JavaWebFrameworkDetector(project_dir=self.project_dir, logger=self.logger)
            analysis.web_framework_analysis = detector.analyze()
            analysis.is_web_application = analysis.web_framework_analysis.has_rest_api or len(analysis.web_framework_analysis.template_engines) > 0
            analysis.is_spring_boot = analysis.web_framework_analysis.primary_framework.value == 'Spring Boot'
        ...
        if analysis.is_spring_boot:
            try:
                spring_analyzer = SpringBootAnalyzer(project_dir=self.project_dir, logger=self.logger)
    ...
    def _build_summary(self, analysis: JavaEcosystemAnalysis) -> Dict[str, str]:
        ...
        summary = {'build_system': analysis.build_system, 'is_java_project': str(analysis.is_java_project)}
    ...
    def build(self, clean: bool=True, skip_tests: bool=False, timeout: int=600):
        """
        ...
        """
        if self.maven_manager:
            return self.maven_manager.build(clean=clean, skip_tests=skip_tests, timeout=timeout)
        elif self.gradle_manager:
            return self.gradle_manager.build(clean=clean, timeout=timeout)
        else:
            raise RuntimeError('No build system detected')

    def run_tests(self, test_class: Optional[str]=None, test_method: Optional[str]=None, timeout: int=300):
        """
        ...
        """
        if self.maven_manager:
            return self.maven_manager.run_tests(test_class=test_class, test_method=test_method, timeout=timeout)
        elif self.gradle_manager:
            return self.gradle_manager.run_tests(test_class=test_class, test_method=test_method, timeout=timeout)
        else:
            raise RuntimeError('No build system detected')
    ...
    def add_dependency(self, group_id: str, artifact_id: str, version: str, scope: str='compile') -> bool:
        """
        ...
        """
        if self.maven_manager:
            return self.maven_manager.add_dependency(group_id, artifact_id, version, scope)
        self.logger.warning('Dependency addition not yet supported for Gradle')
        return False
if __name__ == '__main__':
    import argparse
    import json
    parser = argparse.ArgumentParser(description='Java Ecosystem Manager for Artemis')
...
def _execute_command(manager, args, parser):
    ...
    """
    COMMAND_HANDLERS = {'analyze': _handle_analyze_command, 'build': _handle_build_command, 'test': _handle_test_command}
...
def _print_analysis_table(analysis):
    ...
    logger.log(f'Java Ecosystem Analysis', 'INFO')
...
def _handle_build_command(manager, args):
    ...
    """
    result = manager.build(clean=not args.no_clean, skip_tests=args.skip_tests)
...
def _handle_test_command(manager, args):
    ...
    """
    result = manager.run_tests(test_class=args.test_class, test_method=args.test_method)
