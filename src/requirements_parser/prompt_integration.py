...
logger = get_logger('prompt_integration')
...
class PromptManagerIntegration:
    ...
    def __init__(self, llm: LLMClient, rag: Optional[Any]=None, verbose: bool=False):
        """
    ...
    def parse_with_prompt_manager(self, raw_text: str, project_name: str) -> Dict[str, Any]:
        """
        ...
        if not self.prompt_manager:
            raise RequirementsParsingError('PromptManager not available', context={'prompt_manager_available': PROMPT_MANAGER_AVAILABLE})
        try:
            import uuid
            ...
            if not prompt_template:
                raise RequirementsParsingError('requirements_structured_extraction prompt not found in RAG', context={'prompt_name': 'requirements_structured_extraction'})
            rendered = self.prompt_manager.render_prompt(prompt=prompt_template, variables={'run_id': run_id, 'correlation_id': correlation_id, 'output_format': 'json', 'user_requirements': raw_text[:10000]})
            response = self.llm.generate_text(messages=[LLMMessage(role='system', content=rendered['system']), LLMMessage(role='user', content=rendered['user'])], temperature=0.3)
            return self._parse_json_response(response.content)
        except RequirementsParsingError:
            raise
        except Exception as e:
            raise RequirementsParsingError(f'Failed to parse requirements with PromptManager: {str(e)}', context={'project_name': project_name}, original_exception=e)
    ...
    def _extract_json_from_markdown(self, response: str, original_error: Exception) -> Dict[str, Any]:
        """Extract JSON from markdown code blocks"""
        json_match = re.search('```json\\n(.*?)\\n```', response, re.DOTALL)
        if not json_match:
            raise RequirementsParsingError(f'LLM returned invalid JSON: {original_error}', context={'response_preview': response[:200]}, original_exception=original_error)
    ...
    def log(self, message: str):
        ...
        if self.verbose:
            ...
            logger.log(f'[PromptManagerIntegration] {message}', 'INFO')