...
class StatePatternRecognizer:
    ...
    def detect_unexpected_state(
        ...
        """
        # Guard: State is expected
        if current_state in expected_states:
            return None

        # State is unexpected - create detailed record
    ...
    def _assess_severity(
        ...
        # Guard: Critical failures
        if "FAILED" in state_upper or "CRITICAL" in state_upper:
            return "critical"

        # Guard: High priority errors
        if "ERROR" in state_upper:
            return "high"

        # Guard: Medium priority - has error message
