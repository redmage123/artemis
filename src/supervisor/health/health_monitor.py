...
class HealthMonitor:
    ...
    def register_agent(
        ...
        """
        agent_info = self.agent_registry.register(
            agent_name,
            agent_type,
            metadata,
            heartbeat_interval
    ...
    def monitor_agent_health(
        ...
        while True:
            ...
            # Check if agent hung (timeout exceeded)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        """
        return {
            **self.agent_registry.stats,
            **self.process_monitor.stats,
            **self.crash_detector.stats,
            "registered_agents_count": self.agent_registry.get_agent_count(),
            "monitored_processes_count": self.process_monitor.get_process_count(),
...
# Backward compatibility: expose original properties
# This allows old code to access internal components directly
# DEPRECATED: Use method interface instead
def _setup_backward_compatibility(monitor: HealthMonitor) -> None:
    ...
    monitor._watchdog_running = False  # Read via watchdog.is_running()
    monitor.stats = monitor.get_statistics()  # Dynamic property
