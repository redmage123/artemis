...
class PDFParser:
    ...
    def _detect_library(self) -> None:
        ...
        if self.verbose:
            print("[PDFParser] No PDF library available. Install: pip install PyPDF2 or pdfplumber")
    ...
    def parse(self, file_path: str) -> str:
        ...
        if not self.is_available():
            raise DocumentReadError(
                "PDF support not available. Install: pip install PyPDF2 or pdfplumber",
                context={"file_path": file_path}
            )

        # Guard: Route to appropriate parser
        ...
        raise DocumentReadError(
            f"Unknown PDF library: {self.library}",
            context={"file_path": file_path, "library": self.library}
        )
