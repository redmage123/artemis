...
class CodeStandardsScanner:
    ...
    def scan_codebase(self) -> Dict[str, List[Violation]]:
        ...
        print(f"   Excluding: {', '.join(self.exclude_dirs)}")
        print()
...
def main():
    ...
    if args.critical_only:
        violations = {
            k: [v for v in vals if v.severity == 'critical']
            for k, vals in violations.items()
        }
        scanner.violations_by_type = violations

    # Print report (inline for now - could extract to reporter module)
    ...
    # Exit code - fail if ANY violations found
