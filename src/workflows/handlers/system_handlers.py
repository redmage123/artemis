...
class CleanupZombieProcessesHandler(WorkflowHandler):
    ...
    def handle(self, context: Dict[str, Any]) -> bool:
        try:
            ...
            logger.log(f'[Workflow] Cleaned up {zombies_cleaned} zombie processes', 'INFO')
            return True
        except Exception as e:
            
            logger.log(f'[Workflow] Failed to cleanup zombies: {e}', 'INFO')
            return False
...
class FixPermissionsHandler(WorkflowHandler):
    ...
    def handle(self, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.log(f'[Workflow] Fixed permissions for {file_path}', 'INFO')
            return True
        except Exception as e:
            
            logger.log(f'[Workflow] Failed to fix permissions: {e}', 'INFO')
            return False