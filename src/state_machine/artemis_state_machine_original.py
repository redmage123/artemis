...
logger = get_logger('artemis_state_machine_original')
...
class ArtemisStateMachine:
    ...
    def __init__(self, card_id: str, state_dir: Optional[str]=None, verbose: bool=True, llm_client: Optional[Any]=None):
        ...
        self.stats = {'total_transitions': 0, 'workflow_executions': 0, 'successful_recoveries': 0, 'failed_recoveries': 0, 'issues_resolved': 0}
        if self.verbose:
    ...
    def _build_transition_rules(self) -> Dict[PipelineState, Set[PipelineState]]:
        ...
        """
        return {PipelineState.IDLE: {PipelineState.INITIALIZING, PipelineState.ABORTED}, PipelineState.INITIALIZING: {PipelineState.RUNNING, PipelineState.FAILED, PipelineState.ABORTED}, PipelineState.RUNNING: {PipelineState.STAGE_RUNNING, PipelineState.PAUSED, PipelineState.COMPLETED, PipelineState.FAILED, PipelineState.DEGRADED, PipelineState.ABORTED}, PipelineState.STAGE_RUNNING: {PipelineState.STAGE_COMPLETED, PipelineState.STAGE_FAILED, PipelineState.STAGE_RETRYING, PipelineState.STAGE_SKIPPED, PipelineState.RUNNING}, PipelineState.STAGE_FAILED: {PipelineState.STAGE_RETRYING, PipelineState.RECOVERING, PipelineState.FAILED, PipelineState.ROLLING_BACK}, PipelineState.RECOVERING: {PipelineState.RUNNING, PipelineState.DEGRADED, PipelineState.FAILED, PipelineState.ROLLING_BACK}, PipelineState.DEGRADED: {PipelineState.RUNNING, PipelineState.COMPLETED, PipelineState.FAILED}, PipelineState.PAUSED: {PipelineState.RUNNING, PipelineState.ABORTED}, PipelineState.ROLLING_BACK: {PipelineState.FAILED, PipelineState.ABORTED}, PipelineState.FAILED: {PipelineState.RECOVERING, PipelineState.ROLLING_BACK, PipelineState.ABORTED}, PipelineState.COMPLETED: set(), PipelineState.ABORTED: set()}
    ...
    def transition(self, to_state: PipelineState, event: EventType, reason: Optional[str]=None, **metadata) -> bool:
        ...
        if to_state not in valid_next_states and to_state != from_state:
            if self.verbose:
                ...
                logger.log(f'[StateMachine] ⚠️  Invalid transition: {from_state.value} → {to_state.value}', 'INFO')
            return False
        ...
        if self.verbose:
            ...
            logger.log(f'[StateMachine] {from_state.value} → {to_state.value} (event: {event.value})', 'INFO')
            if reason:
                
                logger.log(f'[StateMachine]    Reason: {reason}', 'INFO')
        self._save_state()
        return True
    ...
    def update_stage_state(self, stage_name: str, state: StageState, **metadata) -> None:
        ...
        if stage_name not in self.stage_states:
            self.stage_states[stage_name] = StageStateInfo(stage_name=stage_name, state=state, start_time=datetime.now())
        else:
            self._update_existing_stage_state(stage_name, state)
        self.stage_states[stage_name].metadata.update(metadata)
        if self.verbose:
            
            logger.log(f"[StateMachine] Stage '{stage_name}' → {state.value}", 'INFO')
        self._save_state()
    ...
    def _update_existing_stage_state(self, stage_name: str, state: StageState) -> None:
        ...
        if not stage_info.start_time:
            return
        stage_info.duration_seconds = (stage_info.end_time - stage_info.start_time).total_seconds()
    ...
    def register_issue(self, issue_type: IssueType, **metadata) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[StateMachine] 🚨 Issue registered: {issue_type.value}', 'INFO')
        if len(self.active_issues) >= 3:
            self.transition(PipelineState.CRITICAL, EventType.HEALTH_CRITICAL, reason=f'{len(self.active_issues)} active issues')
        elif len(self.active_issues) >= 1:
            self.transition(PipelineState.DEGRADED_HEALTH, EventType.HEALTH_DEGRADED, reason=f'{len(self.active_issues)} active issues')
    ...
    def resolve_issue(self, issue_type: IssueType) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[StateMachine] ✅ Issue resolved: {issue_type.value}', 'INFO')
    ...
    def _restore_health_if_cleared(self) -> None:
        ...
        if len(self.active_issues) != 0:
            return
        self.transition(PipelineState.HEALTHY, EventType.HEALTH_RESTORED, reason='All issues resolved')
    ...
    def execute_workflow(self, issue_type: IssueType, context: Optional[Dict[str, Any]]=None) -> bool:
        ...
        if not workflow:
            return False
        execution = WorkflowExecution(workflow_name=workflow.name, issue_type=issue_type, start_time=datetime.now())
        self.stats['workflow_executions'] += 1
        if self.verbose:
            
            logger.log(f'[StateMachine] 🔧 Executing workflow: {workflow.name}', 'INFO')
        try:
            for action in workflow.actions:
                if self.verbose:
                    
                    logger.log(f'[StateMachine]    Action: {action.action_name}', 'INFO')
            ...
            if self.verbose:
                ...
                logger.log(f'[StateMachine] ❌ Workflow error: {e}', 'INFO')
    ...
    def _get_or_generate_workflow(self, issue_type: IssueType, context: Dict[str, Any]) -> Optional[Workflow]:
        ...
        if self.verbose:
            ...
            logger.log(f'[StateMachine] ⚠️  No workflow registered for {issue_type.value}', 'INFO')
        generated_workflow = self._generate_workflow_with_llm(issue_type, context)
        if not generated_workflow:
            return None
        if self.verbose:
            
            logger.log(f'[StateMachine] ✅ Generated new workflow using LLM: {generated_workflow.name}', 'INFO')
        return generated_workflow

    def _handle_workflow_action_failure(self, execution: WorkflowExecution, workflow: Workflow, action: WorkflowAction) -> bool:
        """Handle failure of a workflow action"""
        if workflow.rollback_on_failure:
            if self.verbose:
                
                logger.log(f'[StateMachine]    Rolling back workflow...', 'INFO')
            self._rollback_workflow(execution, workflow)
        execution.success = False
        execution.end_time = datetime.now()
        self.workflow_history.append(execution)
        self.stats['failed_recoveries'] += 1
        self.transition(workflow.failure_state, EventType.RECOVERY_FAIL, reason=f'Workflow {workflow.name} failed at action {action.action_name}')
        return False

    def _complete_workflow_successfully(self, execution: WorkflowExecution, workflow: Workflow, issue_type: IssueType) -> bool:
        ...
        if self.verbose:
            ...
            logger.log(f'[StateMachine] ✅ Workflow completed: {workflow.name}', 'INFO')
        return True

    def _execute_action(self, action: WorkflowAction, context: Dict[str, Any]) -> bool:
        """
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self.verbose:
                    ...
                    logger.log(f'[StateMachine]       Retry {attempt + 1}/{action.max_retries}', 'INFO')
                time.sleep(RETRY_BACKOFF_FACTOR ** attempt)
            except Exception as e:
                if self.verbose:
                    
                    logger.log(f'[StateMachine]       Error: {e}', 'INFO')
                should_retry = action.retry_on_failure and attempt < max_attempts - 1
                if not should_retry:
                    return False
                time.sleep(RETRY_BACKOFF_FACTOR ** attempt)
    ...
    def _rollback_workflow(self, execution: WorkflowExecution, workflow: Workflow) -> None:
        """
        ...
        if self.verbose:
            ...
            logger.log(f'[StateMachine] 🔄 Rolling back workflow: {workflow.name}', 'INFO')
    ...
    def _rollback_single_action(self, action: Optional[WorkflowAction], action_name: str) -> None:
        ...
        try:
            ...
            if self.verbose:
                ...
                logger.log(f'[StateMachine]    Rolled back: {action_name}', 'INFO')
        except Exception as e:
            if self.verbose:
                
                logger.log(f'[StateMachine]    Rollback failed for {action_name}: {e}', 'INFO')
    ...
    def get_snapshot(self) -> PipelineSnapshot:
        ...
        """
        return PipelineSnapshot(state=self.current_state, timestamp=datetime.now(), card_id=self.card_id, stages=dict(self.stage_states), active_stage=self.active_stage, health_status=self._compute_health_status(), circuit_breakers_open=[name for name, info in self.stage_states.items() if info.state == StageState.CIRCUIT_OPEN], active_issues=list(self.active_issues))
    ...
    def _save_state(self) -> None:
        ...
        state_data = {'state': snapshot.state.value, 'timestamp': snapshot.timestamp.isoformat(), 'card_id': snapshot.card_id, 'active_stage': snapshot.active_stage, 'health_status': snapshot.health_status, 'circuit_breakers_open': snapshot.circuit_breakers_open, 'active_issues': [issue.value for issue in snapshot.active_issues], 'stages': {name: {'stage_name': info.stage_name, 'state': info.state.value, 'start_time': info.start_time.isoformat() if info.start_time else None, 'end_time': info.end_time.isoformat() if info.end_time else None, 'duration_seconds': info.duration_seconds, 'retry_count': info.retry_count, 'error_message': info.error_message, 'metadata': info.metadata} for name, info in snapshot.stages.items()}}
    ...
    def _register_default_workflows(self) -> None:
        ...
        self.checkpoint_manager = CheckpointManager(card_id=self.card_id, verbose=self.verbose)
        if self.verbose:
            
            logger.log(f'[StateMachine] Registered {len(self.workflows)} recovery workflows', 'INFO')
    ...
    def _generate_workflow_with_llm(self, issue_type: IssueType, context: Dict[str, Any]) -> Optional[Workflow]:
        ...
        if not self.llm_client:
            if self.verbose:
                ...
                logger.log(f'[StateMachine] ⚠️  Cannot generate workflow - no LLM client available', 'INFO')
            return None
        if self.verbose:
            
            logger.log(f'[StateMachine] 🤖 Consulting LLM to generate workflow for {issue_type.value}...', 'INFO')
        try:
            ...
            if self.verbose:
                ...
                logger.log(f"[StateMachine] ✅ Generated workflow '{workflow.name}' with {len(workflow.actions)} actions", 'INFO')
            return workflow
        except Exception as e:
            if self.verbose:
                
                logger.log(f'[StateMachine] ❌ Failed to generate workflow: {e}', 'INFO')
            return None
    ...
    def _parse_llm_workflow_response(self, response: str) -> Optional[Dict[str, Any]]:
        ...
        if not json_match:
            if self.verbose:
                ...
                logger.log(f'[StateMachine] ⚠️  LLM response did not contain valid JSON', 'INFO')
            return None
    ...
    def push_state(self, state: PipelineState, context: Optional[Dict[str, Any]]=None) -> None:
        ...
        if not hasattr(self, '_state_stack'):
            self._state_stack = []
        self._state_stack.append({'state': state, 'timestamp': datetime.now(), 'context': context or {}})
        if self.verbose:
            
            logger.log(f'[StateMachine] Pushed state onto stack: {state.value} (depth: {len(self._state_stack)})', 'INFO')
    ...
    def pop_state(self) -> Optional[Dict[str, Any]]:
        ...
        if self.verbose:
            ...
            logger.log(f"[StateMachine] Popped state from stack: {popped['state'].value} (depth: {len(self._state_stack)})", 'INFO')
        return popped
    ...
    def rollback_to_state(self, target_state: PipelineState) -> bool:
        ...
        if not rollback_steps:
            if self.verbose:
                ...
                logger.log(f'[StateMachine] ⚠️  State {target_state.value} not found in stack', 'INFO')
            return False
    ...
    def _execute_rollback_to_state(self, target_state: PipelineState, rollback_steps: List[Dict[str, Any]]) -> bool:
        """Execute the rollback to target state"""
        if self.verbose:
            
            logger.log(f'[StateMachine] Rolling back {len(rollback_steps)} states to {target_state.value}', 'INFO')
    ...
    def create_checkpoint(self, total_stages: int) -> None:
        ...
        if hasattr(self, 'checkpoint_manager'):
            self.checkpoint_manager.create_checkpoint(total_stages=total_stages, execution_context={'card_id': self.card_id, 'current_state': self.current_state.value})
