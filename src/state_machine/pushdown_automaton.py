...
logger = get_logger('pushdown_automaton')
...
class PushdownAutomaton:
    ...
    def push_state(self, state: PipelineState, context: Optional[Dict[str, Any]]=None) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[PushdownAutomaton] Pushed state: {state.value} (depth: {len(self._state_stack)})', 'INFO')
    ...
    def pop_state(self) -> Optional[Dict[str, Any]]:
        ...
        if self.verbose:
            ...
            logger.log(f"[PushdownAutomaton] Popped state: {popped['state'].value} (depth: {len(self._state_stack)})", 'INFO')
        return popped
    ...
    def _execute_rollback(self, rollback_steps: List[Dict[str, Any]], target_state: PipelineState) -> None:
        """Execute the rollback to target state"""
        if self.verbose:
            
            logger.log(f'[PushdownAutomaton] Rolling back {len(rollback_steps)} states to {target_state.value}', 'INFO')
    ...
    def _log_state_not_found(self, target_state: PipelineState) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[PushdownAutomaton] ⚠️  State {target_state.value} not found in stack', 'INFO')
    ...
    def clear_stack(self) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[PushdownAutomaton] Stack cleared', 'INFO')
    ...
    def get_stack_snapshot(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            Copy of state stack
        """
        return [{'state': entry['state'], 'timestamp': entry['timestamp'], 'context': entry['context'].copy()} for entry in self._state_stack]