...
logger = get_logger('state_persistence')
...
class StatePersistence:
    ...
    def _convert_snapshot_to_json(self, snapshot: PipelineSnapshot) -> Dict[str, Any]:
        ...
        """
        return {'state': snapshot.state.value, 'timestamp': snapshot.timestamp.isoformat(), 'card_id': snapshot.card_id, 'active_stage': snapshot.active_stage, 'health_status': snapshot.health_status, 'circuit_breakers_open': snapshot.circuit_breakers_open, 'active_issues': [issue.value for issue in snapshot.active_issues], 'stages': self._convert_stages_to_json(snapshot.stages)}

    def _convert_stages_to_json(self, stages: Dict[str, StageStateInfo]) -> Dict[str, Dict[str, Any]]:
        """Convert stage states to JSON-serializable format"""
        return {name: {'stage_name': info.stage_name, 'state': info.state.value, 'start_time': info.start_time.isoformat() if info.start_time else None, 'end_time': info.end_time.isoformat() if info.end_time else None, 'duration_seconds': info.duration_seconds, 'retry_count': info.retry_count, 'error_message': info.error_message, 'metadata': info.metadata} for name, info in stages.items()}
    ...
    def load_snapshot(self) -> Dict[str, Any]:
        ...
        if not state_file.exists():
            if self.verbose:
                ...
                logger.log(f'[StatePersistence] No state file found: {state_file}', 'INFO')
            return {}
        try:
            ...
            if self.verbose:
                ...
                logger.log(f'[StatePersistence] ⚠️  Failed to load state: {e}', 'INFO')
            return {}
    ...
    def delete_snapshot(self) -> None:
        ...
        try:
            ...
            if self.verbose:
                ...
                logger.log(f'[StatePersistence] Deleted state file: {state_file}', 'INFO')
        except Exception as e:
            if self.verbose:
                
                logger.log(f'[StatePersistence] ⚠️  Failed to delete state: {e}', 'INFO')