...
class WorkflowExecutor:
    ...
    def __init__(self, workflows: Dict[IssueType, Workflow], verbose: bool=True) -> None:
        """
        ...
        self.stats = {'workflow_executions': 0, 'successful_recoveries': 0, 'failed_recoveries': 0, 'issues_resolved': 0}
    ...
    def register_issue(self, issue_type: IssueType, **metadata) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[WorkflowExecutor] 🚨 Issue registered: {issue_type.value}', 'INFO')
    ...
    def resolve_issue(self, issue_type: IssueType) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[WorkflowExecutor] ✅ Issue resolved: {issue_type.value}', 'INFO')
    ...
    def execute_workflow(self, issue_type: IssueType, context: Optional[Dict[str, Any]]=None) -> bool:
        ...
        if self.verbose:
            ...
            logger.log(f'[WorkflowExecutor] 🔧 Executing workflow: {workflow.name}', 'INFO')
        try:
    ...
    def _execute_workflow_actions(self, workflow: Workflow, execution: WorkflowExecution, context: Dict[str, Any]) -> bool:
        """Execute all workflow actions in sequence"""
        for action in workflow.actions:
            if self.verbose:
                
                logger.log(f'[WorkflowExecutor]    Action: {action.action_name}', 'INFO')
    ...
    def _attempt_action(self, action: WorkflowAction, context: Dict[str, Any], attempt: int, max_attempts: int) -> bool:
        ...
        try:
            ...
            if should_retry and self.verbose:
                ...
                logger.log(f'[WorkflowExecutor]       Retry {attempt + 1}/{action.max_retries}', 'INFO')
            return False
        except Exception as e:
            if self.verbose:
                
                logger.log(f'[WorkflowExecutor]       Error: {e}', 'INFO')
            return False

    def _handle_action_failure(self, execution: WorkflowExecution, workflow: Workflow, action: WorkflowAction) -> bool:
    ...
    def _complete_workflow_successfully(self, execution: WorkflowExecution, workflow: Workflow) -> bool:
        ...
        if self.verbose:
            ...
            logger.log(f'[WorkflowExecutor] ✅ Workflow completed: {workflow.name}', 'INFO')
        return True

    def _handle_workflow_exception(self, execution: WorkflowExecution, workflow: Workflow, error: Exception) -> bool:
        """Handle workflow execution exception"""
        if self.verbose:
            
            logger.log(f'[WorkflowExecutor] ❌ Workflow error: {error}', 'INFO')
        execution.success = False
        execution.end_time = datetime.now()
    ...
    def _rollback_workflow(self, execution: WorkflowExecution, workflow: Workflow) -> None:
        ...
        if self.verbose:
            ...
            logger.log(f'[WorkflowExecutor] 🔄 Rolling back workflow: {workflow.name}', 'INFO')
    ...
    def _rollback_single_action(self, action: Optional[WorkflowAction], action_name: str) -> None:
        ...
        try:
            ...
            if self.verbose:
                ...
                logger.log(f'[WorkflowExecutor]    Rolled back: {action_name}', 'INFO')
        except Exception as e:
            if self.verbose:
                
                logger.log(f'[WorkflowExecutor]    Rollback failed for {action_name}: {e}', 'INFO')

    def _create_execution_record(self, workflow: Workflow, issue_type: IssueType) -> WorkflowExecution:
        """Create workflow execution record"""
        return WorkflowExecution(workflow_name=workflow.name, issue_type=issue_type, start_time=datetime.now())

    def _log_missing_workflow(self, issue_type: IssueType) -> None:
        """Log missing workflow warning"""
        if self.verbose:
            
            logger.log(f'[WorkflowExecutor] ⚠️  No workflow registered for {issue_type.value}', 'INFO')
