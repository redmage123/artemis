...
class ContentGenerationHelper:
    ...
    def find_reference_implementations(self, task_type: str, min_quality:
        float=7.0, top_k: int=3) ->List[Dict]:
        """
        ...
        """
        query = (
            f'high quality {task_type} reference implementation example best practices'
            )
        results = self.rag.search_code_examples(query=query, top_k=top_k)
        filtered = [r for r in results if r.get('metadata', {}).get(
            'quality_score', 0) >= min_quality]
        logger.log(
            f'🔍 Found {len(filtered)} reference implementations for {task_type}'
            , 'INFO')
        return filtered
    ...
    def extract_domain_knowledge(self, domain: str, top_k: int=5) ->Dict:
        ...
        """
        queries = [f'{domain} features capabilities',
            f'{domain} architecture components',
            f'{domain} pipeline stages workflow']
        knowledge = {'features': [], 'architecture': [], 'stages': [],
            'examples': []}
        for query in queries:
            results = self.rag.query_similar(query_text=query, top_k=top_k)
            for result in results:
                content = result.get('content', '')
                metadata = result.get('metadata', {})
                if 'feature' in query.lower():
                    knowledge['features'].append({'description': content[:
                        300], 'source': metadata.get('task_title', 'Unknown')})
                if 'architecture' in query.lower():
                    knowledge['architecture'].append({'description':
                        content[:300], 'source': metadata.get('task_title',
                        'Unknown')})
                if 'stage' in query.lower() or 'pipeline' in query.lower():
                    knowledge['stages'].append({'description': content[:300
    ...
    def generate_content_brief(self, task_description: str, requirements:
        Dict, task_type: Optional[str]=None) ->str:
        """
        ...
        references = self.find_reference_implementations(task_type,
            min_quality=7.0, top_k=2)
        domain = self._extract_domain(task_description)
        knowledge = self.extract_domain_knowledge(domain, top_k=3
            ) if domain else None
        brief = f"""
...
"""
        if references:
            ...
            for i, ref in enumerate(references, 1):
                ...
                brief += (
                    f"   - Quality: {metadata.get('quality_score', 'N/A')}/10\n"
                    )
                brief += (
                    f"   - Lines: {metadata.get('lines_of_code', 'N/A')}\n")
                brief += (
                    f"   - Description: {metadata.get('description', 'N/A')[:200]}...\n\n"
                    )
        if knowledge and (knowledge['features'] or knowledge['architecture'
            ] or knowledge['stages']):
        ...
        if 'functional' in requirements:
            brief += (
                f"**Functional Requirements:** {len(requirements['functional'])}\n"
                )
        if 'non_functional' in requirements:
            brief += (
                f"**Non-Functional Requirements:** {len(requirements['non_functional'])}\n"
                )
    ...
    def _extract_domain(self, task_description: str) ->Optional[str]:
        ...
        desc_lower = task_description.lower()
        domains = ['artemis', 'authentication', 'authorization',
            'dashboard', 'presentation', 'api', 'database', 'frontend',
            'backend']
