...
class CodeReviewStage(PipelineStage, SupervisedStageMixin, DebugMixin):
    ...
    def __init__(self, messenger: AgentMessenger, rag: RAGAgent, logger:
        LoggerInterface, llm_provider: Optional[str]=None, llm_model:
        Optional[str]=None, observable: Optional[PipelineObservable]=None,
        supervisor: Optional['SupervisorAgent']=None, code_review_dir:
        Optional[str]=None, ai_service: Optional['AIQueryService']=None):
        """
        ...
        SupervisedStageMixin.__init__(self, supervisor=supervisor,
            stage_name='CodeReviewStage', heartbeat_interval=20)
        ...
        self.llm_provider = llm_provider or os.getenv('ARTEMIS_LLM_PROVIDER',
            'openai')
        self.llm_model = llm_model or os.getenv('ARTEMIS_LLM_MODEL')
        self.code_review_dir = self._resolve_code_review_directory(
            code_review_dir)
        self.coordinator = MultiDeveloperReviewCoordinator(logger=logger,
            llm_provider=self.llm_provider, llm_model=self.llm_model,
            rag_agent=rag, code_review_dir=self.code_review_dir,
            debug_component=self)
        self.aggregator = ReviewResultAggregator(logger=logger)
        self.notification_manager = ReviewNotificationManager(observable=
            observable, messenger=messenger, logger=logger)
        self.storage_handler = ReviewStorageHandler(rag=rag, logger=logger)
        self.refactoring_generator = RefactoringSuggestionGenerator(rag=rag,
            logger=logger)

    def _resolve_code_review_directory(self, code_review_dir: Optional[str]
        ) ->str:
        """
        ...
        code_review_dir = os.getenv('ARTEMIS_CODE_REVIEW_DIR',
    ...
    def execute(self, card: Dict, context: Dict) ->Dict:
        ...
        """
        metadata = {'task_id': card.get('card_id'), 'stage': 'code_review'}
    ...
    def _do_code_review(self, card: Dict, context: Dict) ->Dict:
        ...
        if adaptive_config:
            ...
            self.logger.log(
                f'🔧 Using adaptive code review depth: {code_review_depth}',
                'INFO')
            self._apply_review_depth_to_coordinator(code_review_depth)
        else:
            code_review_depth = 'standard'
        self.debug_log('Starting code review', card_id=card_id, task_title=
            task_title, review_depth=code_review_depth)
        self.update_progress({'step': 'starting', 'progress_percent': 5})
        developers = context.get('developers', [])
        if not developers:
            return self._handle_no_developers()
        self.logger.log(
            f'Reviewing {len(developers)} developer implementation(s)', 'INFO')
        self.debug_log('Developers found for review', developer_count=len(
            developers), developer_names=[(d if isinstance(d, str) else d.
            get('name', 'unknown')) for d in developers])
        self.update_progress({'step': 'initializing_reviews',
            'progress_percent': 10})
        review_results = self.coordinator.review_all_developers(developers=
            developers, card_id=card_id, task_title=task_title,
            task_description=task_description, progress_callback=self.
            update_progress, notification_manager=self.notification_manager)
        aggregated = self.aggregator.aggregate_reviews(review_results)
        self.update_progress({'step': 'summarizing_results',
            'progress_percent': 85})
        self.debug_dump_if_enabled('dump_review_results',
            'Code Review Results', {'review_count': len(review_results),
            'critical_issues': aggregated['total_critical_issues'],
            'high_issues': aggregated['total_high_issues'], 'all_pass':
            aggregated['all_reviews_pass'], 'reviews': review_results})
        self._log_review_summary(aggregated)
        self.update_progress({'step': 'determining_status',
            'progress_percent': 95})
        ...
        result = {'stage': 'code_review', 'status': stage_status, 'reviews':
            review_results, 'total_critical_issues': aggregated[
            'total_critical_issues'], 'total_high_issues': aggregated[
            'total_high_issues'], 'all_reviews_pass': aggregated[
            'all_reviews_pass'], 'implementations_reviewed': len(
            review_results)}
    ...
    def _handle_no_developers(self) ->Dict:
        ...
        """
        self.logger.log('No developer implementations found to review',
            'WARNING')
        self.debug_log('No developers to review', developer_count=0)
        return {'stage': 'code_review', 'status': 'SKIPPED', 'reason':
            'No implementations to review'}
    ...
    def _log_review_summary(self, aggregated: Dict) ->None:
        ...
        self.logger.log(
            f"Implementations Reviewed: {aggregated['review_count']}", 'INFO')
        self.logger.log(
            f"Total Critical Issues: {aggregated['total_critical_issues']}",
            'INFO')
        self.logger.log(f"Total High Issues: {aggregated['total_high_issues']}"
            , 'INFO')
    ...
    def _determine_stage_status(self, aggregated: Dict) ->str:
        ...
        if total_critical > 0:
            ...
            return 'FAIL'
        if not all_pass:
            self.logger.log('⚠️  Code review completed with warnings',
                'WARNING')
    ...
    def _generate_refactoring_if_needed(self, stage_status: str,
        review_results: list, card_id: str, task_title: str) ->Optional[str]:
        """
        ...
        if stage_status not in ['FAIL', 'NEEDS_IMPROVEMENT']:
            return None
        return self.refactoring_generator.generate_suggestions(review_results
            =review_results, card_id=card_id, task_title=task_title)

    def _configure_quick_review(self):
        """Configure coordinator for quick code review."""
        ...
        self.logger.log(
            '   Quick review: Critical issues only, skip advanced validation',
            'INFO')

    def _configure_thorough_review(self):
        """Configure coordinator for thorough code review."""
        self.coordinator.enable_code_standards = True
    ...
    def _configure_standard_review(self):
        """Configure coordinator for standard code review."""
        self.coordinator.enable_code_standards = True
        ...
        self.coordinator.advanced_validation_reviewer.enable_property_tests = False
        self.logger.log(
            '   Standard review: Balanced checks, warning+ severity',
            'INFO')
    ...
    def _apply_review_depth_to_coordinator(self, code_review_depth: str) ->None:
        ...
        """
        # Strategy pattern: dispatch to appropriate configuration method
        review_configurators = {
            'quick': self._configure_quick_review,
            'thorough': self._configure_thorough_review,
            'standard': self._configure_standard_review
        }

        configurator = review_configurators.get(code_review_depth, self._configure_standard_review)
        configurator()
