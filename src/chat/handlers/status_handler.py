...
class StatusHandler:
    ...
    def handle(
        ...
        # Guard clause - request card ID if not provided
        if not card_id:
    ...
    def _get_status(self, card_id: str) -> str:
        ...
        try:
            ...
            if self.verbose:
                print(f"[StatusHandler] Status check failed: {e}")
    ...
    def _format_progress(self, progress: Dict[str, Any]) -> str:
        ...
        # Dispatch table for progress ranges
        progress_messages = [
            (100, lambda: f"That one's done! Finished all {total} stages."),
            (75, lambda: f"Almost there - {completed}/{total} stages done. Working on {current} now."),
            (50, lambda: f"Making good progress. {completed}/{total} stages complete, currently handling {current}."),
            (25, lambda: f"Getting started. Finished {completed}/{total} stages, now on {current}."),
            (0, lambda: f"Just started - {completed}/{total} stages done. Currently working through {current}.")
        ]

        # Find appropriate message (early return on first match)
