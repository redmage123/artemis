...
logger = get_logger('pipeline_persistence')
...
from persistence_store import PersistenceStoreInterface, PersistenceStoreFactory, PipelineState, StageCheckpoint
...
class PipelinePersistenceManager:
    ...
    def __init__(self, card_id: str, store: Optional[PersistenceStoreInterface]=None, auto_save: bool=True):
        """
    ...
    def _load_or_create_state(self) -> PipelineState:
        ...
        if existing:
            ...
            return PipelineState(card_id=self.card_id, status='running', current_stage=None, stages_completed=[], stage_results={}, developer_results=[], metrics={}, created_at=datetime.utcnow().isoformat() + 'Z', updated_at=datetime.utcnow().isoformat() + 'Z')
    ...
    def mark_stage_started(self, stage_name: str):
        ...
        self.state.updated_at = datetime.utcnow().isoformat() + 'Z'
        checkpoint = StageCheckpoint(card_id=self.card_id, stage_name=stage_name, status='started', started_at=datetime.utcnow().isoformat() + 'Z', completed_at=None, result={})
    ...
    def mark_stage_completed(self, stage_name: str, result: Dict):
        ...
        self.state.updated_at = datetime.utcnow().isoformat() + 'Z'
        checkpoint = StageCheckpoint(card_id=self.card_id, stage_name=stage_name, status='completed', started_at=self.state.updated_at, completed_at=datetime.utcnow().isoformat() + 'Z', result=result)
        self.store.save_stage_checkpoint(checkpoint)
        if self.auto_save:
            self.store.save_pipeline_state(self.state)
    ...
    def mark_stage_failed(self, stage_name: str, error: str):
        ...
        self.state.updated_at = datetime.utcnow().isoformat() + 'Z'
        checkpoint = StageCheckpoint(card_id=self.card_id, stage_name=stage_name, status='failed', started_at=self.state.updated_at, completed_at=datetime.utcnow().isoformat() + 'Z', result={}, error=error)
        self.store.save_stage_checkpoint(checkpoint)
        if self.auto_save:
            self.store.save_pipeline_state(self.state)
    ...
    def get_statistics(self) -> Dict:
        ...
        return {'card_id': self.card_id, 'status': self.state.status, 'stages_completed': len(self.state.stages_completed), 'total_checkpoints': len(checkpoints), 'created_at': self.state.created_at, 'updated_at': self.state.updated_at, 'completed_at': self.state.completed_at, 'duration_seconds': self._calculate_duration()}
...
if __name__ == '__main__':
    ...
    logger.log('Pipeline Persistence Manager - Example Usage', 'INFO')
    
    logger.log('=' * 70, 'INFO')
    manager = PipelinePersistenceManager(card_id='test-card-002')
    
    logger.log('\n1. Starting pipeline...', 'INFO')
    manager.mark_pipeline_started()
    
    logger.log('2. Starting stage: project_analysis...', 'INFO')
    manager.mark_stage_started('project_analysis')
    
    logger.log('3. Completing stage: project_analysis...', 'INFO')
    manager.mark_stage_completed('project_analysis', {'issues': 0})
    
    logger.log('4. Starting stage: architecture...', 'INFO')
    manager.mark_stage_started('architecture')
    
    logger.log('5. Completing stage: architecture...', 'INFO')
    manager.mark_stage_completed('architecture', {'adr': 'ADR-001.md'})
    
    logger.log('6. Saving developer results...', 'INFO')
    manager.save_developer_results([{'developer': 'developer-a', 'score': 85}, {'developer': 'developer-b', 'score': 90}])
    
    logger.log('7. Completing pipeline...', 'INFO')
    manager.mark_pipeline_completed()
    stats = manager.get_statistics()
    
    logger.log(f'\n✅ Pipeline statistics:', 'INFO')
    
    logger.log(f"   Status: {stats['status']}", 'INFO')
    
    logger.log(f"   Stages completed: {stats['stages_completed']}", 'INFO')
    
    logger.log(f"   Total checkpoints: {stats['total_checkpoints']}", 'INFO')
    
    logger.log(f"   Duration: {stats['duration_seconds']:.2f} seconds", 'INFO')
    resumable = get_resumable_pipelines()
    
    logger.log(f'\n✅ Resumable pipelines: {resumable}', 'INFO')
    
    logger.log('\n' + '=' * 70, 'INFO')
    
    logger.log('✅ Pipeline persistence working correctly!', 'INFO')