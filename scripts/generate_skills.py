#!/usr/bin/env python3
"""
Generate SKILL.md Files for Artemis Agents

This script automatically generates comprehensive SKILL.md documentation files
for all Artemis agent modules. These files provide standardized documentation
including purpose, usage examples, configuration, and integration details.

Purpose:
    Automates the creation of agent documentation to ensure consistency and
    completeness across all agent modules in the Artemis pipeline.

Why This Exists:
    Manual documentation creation is error-prone and inconsistent. This script
    ensures every agent has complete, standardized documentation following
...
def generate_skill_file(agent_key: str, agent_data: dict) -> str:
    """
    Generate SKILL.md content for a specific agent.

    Args:
        agent_key: Identifier key for the agent (e.g., "planning_poker")
        agent_data: Dictionary containing agent metadata including:
            - name: Display name
    ...
    Returns:
        str: Formatted SKILL.md content ready to write to file

    Why This Approach:
        Uses template-based generation to ensure all agents have consistent
        documentation structure while allowing customization for agent-specific
        details like usage examples and configuration.
    """
...
def main():
    """
    Main entry point for SKILL.md generation.

    Process:
    1. Iterate through all defined agents in AGENTS dictionary
    2. Generate formatted SKILL.md content for each agent
    3. Write content to appropriately named file
    4. Report progress to console

    Why This Design:
        Centralized generation ensures all agent documentation is created
        in a single pass with consistent formatting and no manual errors.
    """
